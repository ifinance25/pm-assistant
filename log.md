...
## [2026-09-08] lint | Чистка проекта: диск, .tgz из git, сжат Pulse

Диск: удалены `.DS_Store` (6 шт), случайный `node_modules/` в корне проекта (без `package.json`, создан запуском npm не из `app/`), `artifacts/designs/archive/exports-tmp/*.png` (временные экспорты pen.dev, уже физически на диске после прошлой чистки git), `archive/Untitled` (5-байтовый случайный файл), `.autopilot/__pycache__/`, `app/.playwright-cli/*.log|*.yml`. Git: `git rm artifacts/pm-assistant-app.tgz artifacts/pm-assistant-deploy.tgz` — попали в первый коммит, нигде не упоминались.

`PM_Assistant.md`: Current Focus сведён к одному актуальному блоку (был на два релиза позади CHANGELOG v0.2.8); Open Questions свёрнут — все 11 пунктов были уже решены; Next Actions сокращён с ~29 до 7 незакрытых пунктов, выполненные убраны как дублирующие `log.md`/`CHANGELOG.md`; в Decisions Log две явно замененные строки помечены `(superseded)` без удаления истории.

Не тронуты как не-мусор при проверке: `artifacts/designs/01-home.pen` и `02-home-transcript-v2.pen` (тронуты в последних двух релизах, на них ссылается AI Brief), `artifacts/audio-regression-2026-09-04/` (нужен для открытой задачи 5.1 — эталонные тексты), `.autopilot/dashboards/*` (правило самого автопилота — не удалять старые снимки).

Next: `git status` показан пользователю, commit не сделан — ждёт явной просьбы.

---

## [2026-09-08] lint | История git переписана: реальные записи и дубли стёрты навсегда

По просьбе пользователя `git-filter-repo` стёр из ВСЕЙ истории (не только текущего дерева): `.autopilot/` (мусор скилла), `artifacts/audio-regression-2026-09-04/*` (реальные аудиозаписи встреч + расшифровки), `artifacts/designs/archive/exports-tmp/*.png` (дубли макетов), `app/.playwright-cli/*` (служебный вывод инструмента). `.gitignore` дополнен: `*.log`, `*.pid`, `__pycache__/`, `*.pyc`, `app/.playwright-cli/`, вся `artifacts/audio-regression-*/`. Все хэши коммитов сменились, история отправлена на GitHub force-push (`origin/main` == `main`). Действие разрешено пользователем явно (защитный фильтр среды сперва заблокировал `git-filter-repo`).

Отдельно зафиксировано: папка проекта физически лежит внутри git-репозитория всего Obsidian-хранилища, чей remote — чужой проект `ArianaTent-MiniApp`. Разбор и предложение architecture — отдельным пунктом, в этот коммит не входит.
...
## [2026-09-06] deploy | v0.2.7 на EC2

Пакетный релиз: календарь + весь накопившийся бэклог (LLM-адаптеры, прогресс Whisper, очередь расшифровок, журнал, статус `waiting_room` с бейджем и календарной кнопкой). Правки за сессию 82 (бейдж/openStatuses) внесены отдельно перед выкладкой. 5 коммитов, push в GitHub, `sync.sh` → предполёт → build на сервере → restart api+worker → `doctor.sh` ок → `/api/health` `workerAlive: true`. Побочная находка: записи 0.2.3–0.2.6 в CHANGELOG.md ссылались на устаревший sha `6ea70da` — `record-release` штамповал HEAD, а не реальный коммит выкладки; на проде календаря не было до этого релиза, хотя запись 0.2.5 его упоминала. Не правил задним числом, только зафиксировал.

## [2026-09-06] autopilot | Адаптивная мобильная вёрстка (код, без деплоя)

Бриф по 6 узлам мобильного макета (`artifacts/designs/02-home-transcript-v2.pen`). Автопилот (`full`, T1, 3 тикета в 2 волны) построил: три общих порога ширины (телефон/планшет/десктоп) в `tokens.css`, нижняя панель вкладок вместо сайдбара на ≤640px (`TabBar` в `Sidebar.tsx`, чисто CSS, без JS/state), реф лоу Главной/Расшифровки/Архива/Календаря под телефон, таблица проектов в Настройках → карточки. 11/11 требований манифеста, 1 вне рамок (месячная сетка календаря — фича, не в коде вообще, см. Next Actions), 5 коммитов (`6c23640`…`1f267a0`), 302/302 тестов. Прогон: `.autopilot/2026-09-06-mobile-responsive/`. Побочная находка и чинка по ходу: `.workspace__main` держал десктопный паддинг на любой ширине — сужен на ≤640px отдельным коммитом (D01). Тест версии в Настройках (`v0.2.7`/`v0.2.8`) несколько раз переезжал из-за параллельной несвязанной задачи пользователя над Настройками/Календарём — синхронизирован с текущим значением, может разъехаться снова не по вине этой сборки.

## [2026-09-08] autopilot | Мобильная вёрстка — итог, бэклог

Слепая проверка (агент без спецификации, только бриф+код) и обновление `CLAUDE.md` — упирались в дневной лимит 2026-09-06, перезапущены и пройдены сегодня. Расхождений с задачей не найдено. По просьбе пользователя месячная сетка календаря (вынесена из объёма мобильной сборки как A01) добавлена отдельным пунктом в Next Actions, рядом с остальными фазами календаря.

## [2026-09-08] artifact | Спека: месячная сетка календаря, отмена A01

В Pencil (`02-home-transcript-v2.pen`) добавлена месячная сетка в десктопный кадр `gsvqb` по образцу мобильного `RfNN3` (перенос блока «Month Calendar» + день 6 переделан из вытянутой таблетки в настоящий круг 34×34). Пользователь попросил перенести это в код `/calendar` с проверкой адаптива. Так как сетка раньше явно вынесена «вне рамок» (A01, `.autopilot/2026-09-06-mobile-responsive/spec.md`), код не начат — сперва оформлена `artifacts/2026-09-08-calendar-month-grid-spec.md` (6 требований, без нового API/БД, стрелки месяца рабочим предположением кликабельны). Ждёт утверждения пользователем.

Next: пользователь подтверждает спеку (или правит открытый вопрос про стрелки) → реализация в `CalendarView.tsx`/`calendar.css`.

## [2026-09-08] session | Месячная сетка календаря реализована (код, без деплоя)

`/autopilot full` в этой сессии оказался не тем инструментом (текущий установленный скилл — «держать PR готовым к мержу», не старый билдер фич по `.autopilot/manifest.md`+`tasks.md`) — сборка сделана напрямую, без отдельной тикет-машины. Пользователь при этом утвердил спеку с двумя правками: клик по дню в сетке фильтрует список встреч ниже (было «вне объёма» в черновике), стрелки месяца рабочие. Библиотека для дат — `date-fns` (единственная новая зависимость; сама вёрстка и стили свои, без готового UI-виджета календаря — в проекте и так не было UI-библиотек).

Код: `MonthGrid.tsx` (новый), `CalendarView.tsx`/`CalendarPage.tsx` (выбор дня, отдельный запрос под выбранный день), `adapters/calendar/google.ts` + `server/routes/calendar.ts` (`GET /api/calendar/events` принимает необязательные `from`/`to`, без них поведение старое — 7 дней), `calendar.css` (`.month-grid*`). Спека: `artifacts/2026-09-08-calendar-month-grid-spec.md` (v1.1).

Побочная находка при визуальной проверке: первая версия CSS растягивала кружок дня почти до 200px на десктопе (`aspect-ratio:1` без верхнего предела в грид-колонке произвольной ширины) — поймано скриншотами в headless-браузере (Playwright, временный venv), исправлено фиксированным размером кружка (36px/32px на ≤640px) с центрированием в колонке. Тесты на момент первой версии всё равно проходили — баг был чисто визуальный, ни один юнит-тест его не поймал.

`/calendar` за реальным логином не открывался (учётных данных нет и не просил) — адаптивность проверена на реальном собранном CSS в изолированной HTML-фикстуре на 320/375/768/1440px, не в живом приложении. 311/311 тестов, `npm run build` чисто. На EC2 не выкатывали — обычный пакетный релиз, когда накопится.

Next: пользователь — живая проверка в браузере с реальным Google-аккаунтом; деплой отдельным пакетным релизом.

## [2026-09-08] session | Чистка каркаса контекста: скиллы, манифесты, правила

Аудит каркаса запроса перепроверен по файлам. Каркас Claude Code: 61,7 КБ → 34,1 КБ на запрос (примерно минус 7k токенов).

Что изменилось:
- `~/.claude/skills`: 153 → 41 скилл, 112 перенесены в `~/.claude/skills-archived/` (ничего не удалено). Описания 29 КБ → 11,4 КБ. Папка `gstack/` оставлена: от неё зависят `browse`, `diagram`, `make-pdf`, `investigate`, `design-review`.
- `Projects/PM Assistant/CLAUDE.md`: 327 → 158 строк. Убрано дублирование vault OS (save/query/ingest/sessions/wikilinks/weekly review). Шаблоны страниц вынесены в `wiki/_conventions.md`. Правило чтения Pulse стало послойным.
- Исправлены устаревшие факты в схеме: 56 тестов вместо 23; `retry` вызывает `enqueueJob`; есть `auth.ts` и вход; `finishJob` ставит `done`; Zoom ходит живым ботом `zoom-bot.ts` и читает `ZOOM_CLIENT_ID`/`ZOOM_CLIENT_SECRET`; список маршрутов дополнен семью роутерами.
- Vault `CLAUDE.md`: 304 → 233 строки, блок Karpathy вынесен в `.cursor/rules/karpathy-coding.mdc` с globs по коду.
- Vault `AGENTS.md`: 304 → 10 строк, ссылка на `CLAUDE.md`.
- Удалён дубль `.cursor/rules/agent-access-mode.mdc`, оставлен домашний.

Следующий шаг: проверить `/context` в новом чате, затем commit.

## [2026-09-12] session | Ветка staging, предрелизная проверка, релиз 0.2.9

Собрана ветка `staging` как отдельный шаг предрелизной проверки: `main` плюс фича-ветки, полный регресс, и только потом merge. Месячная сетка календаря до этого вообще не была закоммичена — оформлена в `feature/calendar-month-grid` (код + документы отдельными коммитами).

Что нашла проверка при зелёных 353 тестах:

- 🔴 потеря записи звонка в конце: куски звука шли в файл без очереди, опоздавший кусок пересоздавал файл через `createWriteStream` и стирал запись;
- 🔴 захват вкладки (цель фазы 3.1) отключал сам себя почти на каждом звонке: сторож тишины стартовал до входа в звонок и через 20 с тишины комнаты ожидания уводил бота на запасной путь без возврата;
- 🟡 остановка записи могла ждать событие `stop` вечно (прежняя пауза 1200 мс как раз это страховала);
- 🟡 `.env.example` глушил новый словарь терминов Whisper: пустое значение переменной означает «отключить»;
- ⚪ ответ `GET /api/settings` нельзя было отправить обратно в `PUT` (400 на пустом Google Client ID);
- ⚪ замер громкости писался в журнал каждые 2 с всю встречу, таймеры не гасли после конца записи.

Корень всех трёх дефектов бота: `app/bot/` не входил ни в `tsconfig` (`include: ["src"]`), ни в `biome` (`includes: ["src/**"]`), ни в тесты. Подключён: линтер смотрит на `bot/**`, добавлен `bot-scripts.test.ts`. Заодно тест версии в Настройках перестал прибивать номер строкой и берёт его из `shared/version.ts` — он падал на каждом подъёме версии.

Решение пользователя по ходу: приложение открыто наружу по домену через HTTPS-прокси, авторизация дальше будет от Supabase. Это снимает блокер фазы 6 (Supadata нужен публичный адрес записи). Пункт про конфликт фазы 6 с правилом «только 127.0.0.1» снят из списка дефектов.

Итог: 360 тестов в 63 файлах, линтер на 186 файлах, сборка, 37 из 37 интеграционных запросов к живому API на отдельном порту с временной базой. Релиз 0.2.9 влит в `main` (`cedb86c`), `main`, `staging` и обе фича-ветки отправлены в GitHub.

Выкатка на EC2 не сделана: запись на прод-сервер заблокирована защитой сеанса, `preflight.sh` при этом прошёл (воркер свободен). Выкатку запускает пользователь.

Next: `sync.sh` и сборка на сервере, `PM_ASSISTANT_COOKIE_SECURE=1` в `/etc/pm-assistant.env`, живой звонок Zoom для приёмки фаз 3-4.

## [2026-09-12] deploy | v0.2.9 на EC2

Пакетный релиз: месячная сетка календаря, фазы качества расшифровки 3-6 и правки предрелизной проверки. Коммит `301b8e3`, выкладка 12:29.

Путь: предполёт зелёный → `sync.sh` → `npm install` + `npm rebuild better-sqlite3` + `npm run build` на сервере под `pm-assistant@0.2.9` → restart api+worker → `doctor.sh` всё ок (оба юнита active, whisper-cli и модель 466M на месте, образ `pm-assistant-zoom-bot` на месте). Заодно на прод добавлен `PM_ASSISTANT_COOKIE_SECURE=1`: приложение теперь снаружи, без флага `Secure` cookie сессии уходила бы по открытому HTTP.

Выкатку запускал пользователь: запись на прод-сервер в этом сеансе агента заблокирована классификатором прав, чтение (`preflight.sh`) проходило.

Приёмки фаз 3-4 по-прежнему нет: правки бота проверены чтением, разбором синтаксиса и тестами, закрепляющими форму кода, но не живым звонком. Первый же звонок покажет и громкость записи, и подписи спикеров.

Next: живой звонок Zoom, Google Cloud Console, эталонные тексты 5.1a-c для WER.
